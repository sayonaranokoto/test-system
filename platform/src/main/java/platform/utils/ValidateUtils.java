...
public class ValidateUtils {
    public static <T> void validate(String type, T responseBody, Expression expression) throws TestException {
    ...
    public static void validateJsonData(String json, Expression expression) throws TestException {
        if (StringUtils.isBlank(expression.getPath())) {
            throw new TestException(TestExceptionEnum.EXPRESSION_PARAM_ERROR);
        }
        String value = JSONPath.read(json, "$." + expression.getPath(), String.class);
        validateStringData(value, expression);
    }

    public static void validateStringData(String str, Expression expression) throws TestException {
        ...
        if (!valid) {
            throw new TestException(TestExceptionEnum.EXPRESSION_NOT_MATCH);
        }
    }

    public static void validateMapData(Map map, Expression expression) throws TestException {
    ...
    private static boolean isRegex(String value, String expressionValue) throws TestException {
        try {
